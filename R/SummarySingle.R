  ...
  for (i in 1:length(trt.lev)) {
    ...
    if (var.class == "ordered.factor") {
      ...
      if (requireNamespace(coin, quietly = TRUE)) {
        ...
        # load from suggested namespace
        coin.pvalue <- getExportedValue("coin", "pvalue")
        coin.cmh_test <- getExportedValue("coin", "cmh_test")

        tt <- coin.pvalue(coin.cmh_test(
          as.formula(paste0(var,' ~ ',subgroup[2])),
          data = tmp.mat))
      } else {
